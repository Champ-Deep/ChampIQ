...
import { getNodeMeta, getRestAction, getConfigSchema, getPopulateEndpoints } from '@/lib/manifest'
...
export function ToolNode({ id, data, selected }: NodeProps) {
  const manifest = data.manifest as ChampIQManifest
  ...
  useEffect(() => {
    const toolId = manifest['x-champiq'].tool_id
  ...
  useJobPolling(runtime.jobId, id, manifest['x-champiq'].tool_id)
  ...
  async function handleAction() {
    ...
    try {
      ...
      const result = await api.runAction(toolId, actionPath, {
        ...inputPayload,
        config: formData,
      })
    ...
    >
      {meta.accepts_input_from.length > 0 && (
        <Handle
          ...
          className="!bg-slate-500 !w-3 !h-3 !border-slate-400"
      ...
      <div
        ...
        aria-label={`${meta.label} node header`}
      >
      ...
      {!collapsed && (
        <div className="px-3 pb-3 space-y-2">
          ...
          <button
            ...
            aria-label={action.button_label}
          >
