...
const ACTION_FIELDS: Record<string, Record<string, FieldDef[]>> = {
  ...
  champvoice: {
    initiate_call: [
      ...
      { key: 'lead_name', label: 'Prospect name', type: 'text', placeholder: '{{item.first_name}} {{item.last_name}}' },
      { key: 'company', label: 'Company', type: 'text', placeholder: '{{item.company}}' },
      { key: 'email', label: 'Email', type: 'text', placeholder: '{{item.email}}' },
      { key: 'agent_id', label: 'Agent ID override (optional)', type: 'text',
        placeholder: 'agent_abc123',
        hint: 'Leave blank to use the credential-level default. Set a different ElevenLabs agent ID here to use a different agent for this specific call type.' },
      { key: 'call_reason', label: 'Call reason', type: 'select',
        options: ['', 'cold_outreach', 'email_follow_up', 'sequence_completed', 'replied_follow_up'],
        hint: 'Injected as ElevenLabs dynamic variable so the agent can tailor its opening.' },
      { key: 'engagement_status', label: 'Engagement status (from ChampGraph)', type: 'text',
        placeholder: '{{prev.engagement_status}}',
        hint: 'Pass the engagement_status from a get_prospect_status node to personalize the call.' },
      { key: 'script', label: 'Script override (optional)', type: 'textarea',
        placeholder: 'Hi {{lead_name}}, I noticed you opened our email about…',
        hint: 'Overrides the agent\'s built-in script. Supports ElevenLabs dynamic variable syntax.' },
      { key: 'dynamic_vars', label: 'Extra dynamic variables (JSON object)', type: 'textarea',
        placeholder: '{"budget":"{{item.budget}}","vertical":"{{item.vertical}}"}',
        hint: 'Additional key-value pairs injected into the ElevenLabs conversation.' },
    ],
    get_call_status: [
